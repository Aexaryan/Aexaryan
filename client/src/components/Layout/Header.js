...
const Header = ({ onMenuClick, user }) => {
  ...
  const getMessagesLink = () => {
    return user.role === 'talent' ? '/talent/messages' : '/director/messages';
  };
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to={getMessagesLink()}
              className="relative flex items-center gap-2 px-3 py-2 text-gray-600 hover:text-black hover:bg-gray-100 rounded-lg transition-all duration-200 group"
              title="پیام‌ها"
            >
              <ChatBubbleLeftIcon className="h-5 w-5" />
              <span className="hidden sm:block text-sm font-medium">پیام‌ها</span>
              {unreadCount > 0 && (
                <>
                  ...
                  <span className="hidden sm:block bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full">
                    {unreadCount > 99 ? '99+' : unreadCount}
                  </span>
                </>
              )}
