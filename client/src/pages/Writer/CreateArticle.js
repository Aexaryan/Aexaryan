...
const CreateArticle = () => {
  ...
  const checkPermission = useCallback(async () => {
    try {
      console.log('Checking article creation permission...');
      const response = await api.get('/writer/can-create-article');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/writer/articles', formDataToSend, {
        headers: {
  ...
  if (!canCreate) {
    console.log('User cannot create articles, redirecting...');
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-semibold text-gray-900 mb-2">شما مجوز ایجاد مقاله ندارید</h2>
          <p className="text-gray-600 mb-4">برای نویسندگان، تایید هویت الزامی است.</p>
          <button
