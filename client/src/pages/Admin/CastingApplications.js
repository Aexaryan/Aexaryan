...
const AdminCastingApplications = () => {
  const { id } = useParams();
  ...
  const fetchCastingApplications = async () => {
    try {
      ...
      const [castingRes, applicationsRes] = await Promise.all([
        api.get(`/admin/castings/${id}`),
        api.get(`/admin/castings/${id}/applications?${params.toString()}`)
      ]);
  ...
  const handleBulkStatusChange = async (newStatus) => {
    ...
    try {
      await api.patch('/admin/applications/bulk/status', {
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-600">
          ...
          <span>کارگردان: {casting.castingDirector?.firstName} {casting.castingDirector?.lastName}</span>
        </div>
      </div>
      ...
      {applications.length > 0 ? (
                ...
                <div className="flex-1">
                  ...
                  {application.directorNotes && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-3">
                      <h4 className="text-sm font-medium text-blue-900 mb-1">یادداشت کارگردان:</h4>
                      <p className="text-sm text-blue-800">{application.directorNotes}</p>
                    </div>
                  )}
                  ...
                  {application.adminNotes && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-3">
                      <h4 className="text-sm font-medium text-yellow-900 mb-1">یادداشت ادمین:</h4>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                    <div className="flex items-center gap-2">
                      <Link
                        to={`/admin/users/${application.talent._id}`}
                        className="btn-outline btn-sm"
                      >
                        مشاهده پروفایل
                      </Link>
        ...
        <div className="text-center py-20">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            {filter === 'all' ? 'هنوز درخواستی دریافت نشده' : 'درخواستی با این وضعیت یافت نشد'}
          </h2>
          <p className="text-gray-600 mb-4">
            {filter === 'all' 
              ? 'هنوز استعدادی برای این کستینگ درخواست ارسال نکرده است'
...
export default AdminCastingApplications;
