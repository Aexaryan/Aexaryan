...
import { PlusIcon, TrashIcon, PhotoIcon, XMarkIcon, ArrowRightIcon } from '@heroicons/react/24/outline';
...
const EditCasting = () => {
  ...
  const fetchCastingData = async () => {
    try {
      ...
      if (casting.castingDirector._id !== user.id) {
        toast.error('شما مجاز به ویرایش این کستینگ نیستید');
        navigate('/director/my-castings');
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(`/castings/${id}`, cleanedFormData, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  // Show message if user is not a casting director
  if (user.role !== 'casting_director') {
    return (
      <div className="text-center py-20">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">دسترسی محدود</h2>
        <p className="text-gray-600">فقط کارگردانان کستینگ می‌توانند کستینگ ویرایش کنند.</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-4 mb-4">
          <button
            onClick={() => navigate('/director/my-castings')}
            className="flex items-center text-gray-600 hover:text-gray-800 transition-colors"
          >
            <ArrowRightIcon className="w-5 h-5 ml-1" />
            بازگشت به کستینگ‌های من
          </button>
        </div>
